...
struct User {
    ...
    var email: String
    var age: Int
    static var currentUser: User = User(userName: "Rajveer", email: "rss@email.com", age: 21)
    static func logIn(user: User) {
        currentUser = user
        print("\(user.userName) has logged in.")
    }
}
print("Current user: \(User.currentUser.userName), \(User.currentUser.email), \(User.currentUser.age)")
 ...
 */
let newUser = User(userName: "anotherUser", email: "another@example.com", age: 25)
